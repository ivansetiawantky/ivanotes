...
\date{Created: 2026年4月17日\\[0.3em]Last Updated: 2026/05/06}
...
\begin{longtable}{p{\cwD} p{\cwE}}
    ...
    \begin{adjustbox}{valign=t,minipage=\cwD}
        \begin{lstlisting}[language={[LaTeX] TeX},numbers=none,frame=none,breaklines=true,aboveskip=0em,belowskip=0em,gobble=12,]
            ...
            \coordinate[label={[label distance=0.2cm] -60:B}] (B) at (3,2);
            \draw[very thin] (A) --(B);
...
\section{「\LaTeX{}で図を直接描けるTikZの使い方５｜領域に色を塗る」}
\label{areacoloring}

第~\ref{areacoloring}章では，\url{https://math-note.xyz/latex/tikz/tikz-color-of-area/}にあるコードを，いくつか試してみる．

\verb|\filldraw| または \verb|\draw[fill=色]| で \verb|\fill|（図形の色塗り）と \verb|\draw| （図形の描画）とを同時にできる．
\begin{lstlisting}[
    language={[LaTeX] TeX},
    firstnumber=1,breaklines=true,gobble=4]
    \begin{tikzpicture}
        ...
        \draw[fill=lightgray,very thick] (A) --(B) --(C) --cycle; %三角形ABCを描き，内部をlightgrayに塗る
    \end{tikzpicture}
\end{lstlisting}
\[
    \begin{tikzpicture}
...
\verb|red!30!blue| みたいに，red を \qty{30}{\percent} と残り blue で混ぜることができる．また \verb|red!30| は，\verb|red!30!white| と同じである．
\begin{lstlisting}[
    language={[LaTeX] TeX},
    firstnumber=1,breaklines=true,gobble=4]
    \begin{tikzpicture}
    ...
    \draw[fill=blue!30,very thick] (A) --(B) --(C) --cycle;
    \coordinate (S1) at (4,0);
    \draw[fill=blue!30!white,very thick] ($(A)+(S1)$) --($(B)+(S1)$) --($(C)+(S1)$) --cycle;
    \coordinate (S2) at (4,-3);
    \draw[fill=blue!30!red,very thick] ($(A)+(S2)$) --($(B)+(S2)$) --($(C)+(S2)$) --cycle;
    \end{tikzpicture}
\end{lstlisting}
\[
    \begin{tikzpicture}
